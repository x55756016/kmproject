...
namespace KMHC.CTMS.Common.Helper
{
    /// <summary>
    /// <para>Copyright (C) 2015 康美健康云服务有限公司版权所有</para>
    /// <para>文 件 名： ConvertHelper.cs</para>
    /// <para>文件功能： 处理数据类型转换，数制转换、编码转换相关的类</para>
    /// <para>开发部门： 平台部</para>
    /// <para>创 建 人： lmf</para>
    ...
    public class ConvertHelper
    {
        ...
        {
            //如果为空则返回默认值
            if (data == null || System.Convert.IsDBNull(data))
            {
                return defValue;
            }
        ...
        public static double ToDouble(object data, int decimals, double defValue)
        {
            ...
            try
            {
                return Math.Round(System.Convert.ToDouble(data), decimals);
            }
            catch
            {
                return defValue;
        ...
        public static DateTime ToDateTime(object data, DateTime defValue)
        {
            ...
            try
            {
                return System.Convert.ToDateTime(data);
            }
            catch
            {
                return defValue;
